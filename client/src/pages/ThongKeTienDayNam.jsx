...
function ThongKeTienDayNam() {
  ...
  const columns = [
    ...
    {
      render: _ => <Button size="small" color="blue" variant="solid" icon={<FontAwesomeIcon icon={faInfo} />}
        onClick={() => {
          console.log(_)
          setSelected(_.id)
        }} />
  ...
  const columns2 = [
    { title: 'Lớp học phần', dataIndex: 'tenLopHocPhan', key: 'tenLopHocPhan' },
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-[1.5fr_1fr] gap-5">
        ...
        <Table className="mb-2" size="small" pagination={{ pageSize: 10 }}
          columns={columns}
          dataSource={allData} />
        <div>
          <h2 className="mb-2 font-bold uppercase">Lớp học phần giảng viên giảng dạy</h2>
          <h2 className="mb-2 font-bold uppercase">{allData.find(i => i.id == selected)?.hoTen}</h2>
